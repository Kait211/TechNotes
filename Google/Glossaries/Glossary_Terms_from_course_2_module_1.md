...
- **Multicast frame:** If the least significant bit in the first octet of a destination address is set to one, it means you're dealing with a multicast frame. A multicast frame is similarly sent to devices on the local network, and each device will accept or discard it depending on criteria aside from its own hardware MAC address
...
- **Network port:** The physical connector used to connect a device to the network. This may be attached directly to a device on a computer network or could also be located on a wall or on a patch panel

- **Network switch:** It is a Level 2 (Data Link) device that can connect many devices so they can communicate. It can inspect the contents of Ethernet frames, determine which system the data is intended for, and send that data only to the intended system
...
- **Preamble:** The first part of an Ethernet frame. It is 8 bytes (64 bits) long and can itself be split into two sections
...
- **Source MAC address:** The hardware address of the device that sent the Ethernet frame or data packet. It follows the Destination MAC Address
...
- **Transmission Control Protocol (TCP):** The data transfer protocol most commonly used in the Transport layer. This protocol requires an established connection between the client and server

- **Transport layer:** The network layer that sorts out which client and server programs are supposed to receive the data
...
- **Unicast transmission:** A transmission that is always meant for just one receiving address

- **User Datagram Protocol (UDP):** A transfer protocol that does not rely on connections. This protocol does not support acknowledgements. With UDP, you simply set a destination port and send the data packet

- **Virtual LAN (VLAN):** A technique that lets you have multiple logical LANs operating on the same physical equipment

- **VLAN header:** A piece of data that indicates what the frame itself is. It is followed by the EtherType field